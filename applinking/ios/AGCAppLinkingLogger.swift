...
let AGCAppLinkingLog = AGCAppLinkingLogger(version: "1.9.0", service: "ReactNativeApplinking")

/// Custom AGCAppLinkingLogger Class to listen api log events from RN Side.
final class AGCAppLinkingLogger {
  ...
  private var prefix: String = GenericFunctions.className(AGCAppLinkingLogger.self)
  ...
  func showInPanel(callTime: Double? = nil,
                           ...
                           type: AGCAppLinkingLogger.InfoType) {
    switch type {
    case .call:
      printCall(name)
      return
