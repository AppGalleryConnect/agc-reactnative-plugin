...
protocol AGCAppLinkingHandling {
...
extension AGCAppLinkingHandling {
  func handle(resolve: @escaping RCTPromiseResolveBlock, _ isSuccess: Bool?=true, _ message: String?=nil) {
    AGCAppLinkingPromise.shared.resolve(resolve: resolve, isSuccess, message)
  }

  func handle<T>(resolve: RCTPromiseResolveBlock, _ instance: T) {
    AGCAppLinkingPromise.shared.resolve(resolve: resolve, instance: instance)
  }

  func fail(with error: Error?, reject: @escaping RCTPromiseRejectBlock){
    AGCAppLinkingPromise.shared.reject(reject: reject, error: error)
  }
}

class AGCAppLinkingPromise {
  static let shared = AGCAppLinkingPromise()
