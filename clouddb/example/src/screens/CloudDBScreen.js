...
import AGCCloudDB, { AGCCloudDBQuery, AGCCloudDBZoneConfig, AGCCloudDBTransaction } from "@hw-agconnect/react-native-clouddb";
...
const CloudDBScreen = (props) => {
    ...
    const executeServerStatusQuery = () => {
        dbZone.executeServerStatusQuery().then(response => {
            showResult(response);
        }).catch(error => {
            showResult(error.message)
        })
    ...
    const setUserKey = () => {
        AGCCloudDB.getInstance().setUserKey("gbkpassword10", "",true)
            .then(res => showResult(res))
            .catch(err => showResult(err.message))
    }
                ...
                <ScrollView>
                    ...
                    {dbZone ?
                        <View>
                            ...
                            />
                            <CustomButton
                                title={"Execute Server Status Query"}
                                onPress={() => executeServerStatusQuery()}
                            />
                        </View>
                        : null}
                    <CustomButton
