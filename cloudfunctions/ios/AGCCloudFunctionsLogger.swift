...
let AGCCloudFunctionsLog = AGCCloudFunctionsLogger(version: "1.4.1.300", service: "ReactNativeCloudFunctions")

/// Custom AGCCloudFunctionsLogger Class to listen api log events from RN Side.
final class AGCCloudFunctionsLogger {
  ...
  private var prefix: String = GenericFunctions.className(AGCCloudFunctionsLogger.self)
  ...
  func showInPanel(callTime: Double? = nil,
                           ...
                           type: AGCCloudFunctionsLogger.InfoType) {
    switch type {
    case .call:
      printCall(name)
      return
